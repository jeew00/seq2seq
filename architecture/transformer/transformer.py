...
class Seq2Seq(nn.Module):
    ...
    def predict(self, x, max_len):
        bs = x.size(0)
        ys = torch.ones(bs, 1, device=self.device, dtype=torch.long).fill_(1) #fill_(sos_idx=1)
        src_mask, tgt_mask = self.make_masks(x, ys)
        enc_src = self.encoder(x, src_mask)
        for i in range(max_len - 1):
            src_mask, tgt_mask = self.make_masks(x, ys)
            out = self.decoder(ys, enc_src, tgt_mask, src_mask)
            _, next_word = torch.max(out[:, -1], dim=-1)
            ys = torch.cat([ys, next_word.unsqueeze(1)], dim=1)
        return ys

    
def create_transformer(input_dim, output_dim, d_model, N, h, d_ff, dropout, device, pad_idx=0):
    enc = Encoder(
        input_dim,
        d_model,
        N,
    ...
    return Seq2Seq(enc, dec, pad_idx, device).to(device)
